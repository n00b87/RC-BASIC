...
{
    //cout << "DBG_MID" << endl;
    if(rc_sid[MID_STR_STRING_STR][0].length() <= (int)rc_nid[MID_STR_START][0])
        return "";
    if((int)(rc_nid[MID_STR_START][0]+rc_nid[MID_STR_N][0]) >= rc_sid[MID_STR_STRING_STR][0].length())
        return rc_sid[MID_STR_STRING_STR][0].substr((int)rc_nid[MID_STR_START][0]);
...
{
    rc_user_n_stack.push(rc_nid[PUSH_N_N][0]);
    return 1;
}

inline int rc_intern_push_s()
{
    rc_user_s_stack.push(rc_sid[PUSH_S_S_STR][0]);
    return 1;
}
...
inline void rc_vm_intern()
{
    ...
    switch(i_val1)
    {
        ...
        case 273:
            ...
            break;
        case 274:
            rc_num_stack[rc_num_stack_index] = rc_intern_system(rc_sid[SYSTEM_SYS_CMD_STR][0]);
            rc_num_stack_index++;
            break;
        case 275:
        ...
        case 301:
            ...
            break;
        case 302:
            rc_num_stack[rc_num_stack_index] = rc_intern_pop_n();
            rc_num_stack_index++;
            break;
        case 303:
            rc_str_stack[rc_str_stack_index] = rc_intern_pop_s();
            rc_str_stack_index++;
            break;
        case 304:
            rc_num_stack[rc_num_stack_index] = rc_intern_n_stack_size();
            rc_num_stack_index++;
            break;
        case 305:
            rc_num_stack[rc_num_stack_index] = rc_intern_s_stack_size();
            rc_num_stack_index++;
            break;
        case 306:
...
int main(int argc, char * argv[])
{
    ...
    //EDITOR CODE--------------------------------------------------
//    fstream dfile;
//    dfile.open("rc_dtr", fstream::in);
//    if(!dfile.is_open())
//       cout << "Could not find dtr file\n";
//
//    string fname = "";
//    getline(dfile, fname);
//    dfile.close();
//
//    chdir(fname.substr(0, fname.find_last_of("\\")).c_str());
    //---------------------------------------------------------------
