...
inline bool rc_media_openWindow_hw(int win_num, string caption, int x, int y, int w, int h, Uint32 flags)
{
    ...
    rc_win[win_num] = SDL_CreateWindow(caption.c_str(), 0, 0, w, h, SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN_DESKTOP);
    if(rc_win[win_num] == NULL)
    {
    ...
    //SDL_ConvertSurfaceFormat(rc_win_surface[win_num], SDL_PIXELFORMAT_ARGB8888, 0);
    ...
    //SDL_GetRendererInfo(rc_win_renderer[win_num], ren_info);
    //if(ren_info->flags & SDL_RENDERER_ACCELERATED)
    //__android_log_write(ANDROID_LOG_ERROR,"RendererInfo: ", "ACCELERATED");
    //SDL_RenderSetLogicalSize(rc_win_renderer[win_num],w,h);
    //__android_log_print(ANDROID_LOG_ERROR, "RC_DEBUG", "Logical Size Error: %s", SDL_GetError());

    rc_backBuffer[win_num] = SDL_CreateTexture(rc_win_renderer[win_num], rc_pformat->format, SDL_TEXTUREACCESS_TARGET, w, h);
    //SDL_SetTextureBlendMode(rc_backBuffer[win_num],SDL_BLENDMODE_BLEND);
...
void rc_media_setWindowBordered(int win_num, SDL_bool bswitch)
{
    if(rc_winCheck(win_num))
...
void rc_media_setScreenViewport(int x, int y, int w, int h)
{
    if(rc_screenCheck(rc_active_screen))
    {
        rc_screen_rect[rc_active_window][rc_active_screen].x = x;
        rc_screen_rect[rc_active_window][rc_active_screen].y = y;
        rc_screen_rect[rc_active_window][rc_active_screen].w = w;
        rc_screen_rect[rc_active_window][rc_active_screen].h = h;
    }
}

void rc_media_getScreenViewport(double * x, double * y, double * w, double * h)
{
    if(rc_screenCheck(rc_active_screen))
    {
        *x = rc_screen_rect[rc_active_window][rc_active_screen].x;
        *y = rc_screen_rect[rc_active_window][rc_active_screen].y;
        *w = rc_screen_rect[rc_active_window][rc_active_screen].w;
...
void rc_media_screenOffset(int x, int y)
{
    if(rc_screenCheck())
    {
        ...
        rc_screenview[rc_active_window][rc_active_screen].y = y;
        rc_screenview[rc_active_window][rc_active_screen].w = rc_screen_rect[rc_active_window][rc_active_screen].w;
        rc_screenview[rc_active_window][rc_active_screen].h = rc_screen_rect[rc_active_window][rc_active_screen].h;
        //SDL_SetClipRect(rc_sscreen[rc_active_window][rc_active_screen], &rc_screenview[rc_active_window][rc_active_screen]);
    }
}

void rc_media_getScreenOffset(double * x, double * y)
{
    if(rc_screenCheck())
    {
        *x = rc_screenview[rc_active_window][rc_active_screen].x;
        *y = rc_screenview[rc_active_window][rc_active_screen].y;
    }
}

void rc_media_getScreenSize(double * w, double * h)
{
    if(rc_screenCheck())
    {
        *w = rc_screen_width[rc_active_window][rc_active_screen];
        *h = rc_screen_height[rc_active_window][rc_active_screen];
    }
}
...
void rc_media_setScreenZ(int z)
{
    if(rc_screenCheck())
    {
        ...
        rc_screen_z[rc_active_window][rc_active_screen] = z;
...
void rc_media_floodFill_sw(int x, int y)
{
    if(rc_screenCheck())
    {
        int s_lock = 0;
        ...
        rc_media_floodFill_sw(x+1, y);
        rc_media_floodFill_sw(x-1, y);
        rc_media_floodFill_sw(x, y+1);
        rc_media_floodFill_sw(x, y-1);
...
void rc_media_loadImage_hw(int slot, string img_file)
{
    ...
    rc_image_width[slot] = image->w;
    rc_image_height[slot] = image->h;
    if(image_cv == NULL)
    {
        cout << "LoadImage Error: " << SDL_GetError() << endl;
...
void rc_media_createImage_hw(int slot, int w, int h, double * pdata, int pdata_size)
{
    if(rc_himage[slot][rc_active_window] != NULL)
    {
    ...
    if(pdata_size < (w*h))
    {
        cout << "CreateImage Error: Buffer exceeds the size of pixel data" << endl;
        return;
    }
...
void rc_media_createImage_Ex_hw(int slot, int w, int h, double * pdata, double color, int pdata_size)
{
	if(rc_himage[slot][rc_active_window] != NULL)
	{
	    cout << "CreateImage Error: Image already exist" << endl;
	...
	SDL_SetColorKey(img_surf, SDL_TRUE, (Uint32)color);
	SDL_DestroyTexture(rc_himage[slot][rc_active_window]);
	rc_himage[slot][rc_active_window] = SDL_CreateTextureFromSurface(rc_win_renderer[rc_active_window], img_surf);
	SDL_SetTextureBlendMode(rc_himage[slot][rc_active_window], SDL_BLENDMODE_BLEND);
...
void rc_media_getImageAlpha_hw(int slot, double * alpha)
{
    if(slot < 0 || slot >= MAX_IMAGES)
    {
...
int rc_getEvents()
{
    ...
    switch(event.type)
    {
        ...
        case SDL_WINDOWEVENT:
            ...
            break;
        case SDL_TEXTINPUT:
            if(rc_textinput_flag == true || !rc_textinput_isActive)
            {
                //cout << "NO GO" << endl;
                break;
            }
...
void rc_media_getMouse(double * x, double * y, double * mb1, double * mb2, double * mb3)
{
    ...
    *mb3 = rc_mbutton3;
    //int x_data, y_data;
    *x = rc_mouseX * rc_mouse_scale_x;
    *y = rc_mouseY * rc_mouse_scale_y;
    return;
//    while(rc_getEvents())
//    {
...
void rc_net_tcp_openSocket(int _socket, string host, Uint16 port)
{
    IPaddress ip;
    if(host.compare("") == 0)
    {
...
void rc_media_loadVideo(string fname)
{
    ...
    {
        decoder = NULL;
        __android_log_print(ANDROID_LOG_ERROR, "RC_DEBUG","LOADVIDEO_FAILED: ");
        return;
    }
